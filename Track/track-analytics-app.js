import { loadTrackAnalyticsData, applyFilters, uniqueValues } from "./track-analytics-data.js";
import { getDefaultState, parseStateFromUrl, writeStateToUrl, buildShareUrl } from "./track-analytics-state.js";
import { buildKpis, buildInsights, compareSlices } from "./track-analytics-insights.js";
import { renderTrendChart, renderCategoryChart, renderCustomChart, renderCoachTrendChart, renderCoachEventChart } from "./track-analytics-charts.js";

const filterIds = [
  "season", "gender", "eventId", "category", "schoolName", "meetId", "athleteName",
  "relayFilter", "markType", "startDate", "endDate", "placementMax",
  "compareBy", "compareEventId", "compareA", "compareB", "coachAthleteName", "coachEventId",
  "chartType", "groupBy", "metric", "topN",
];
...
function setOptions(selectId, options, valueKey = "value", labelKey = "label", includeAll = true, allLabel = "All") {
  const node = el(selectId);
  if (!node) return;
  const initial = includeAll ? `<option value="">${allLabel}</option>` : "";
  node.innerHTML = initial + options.map((opt) => `<option value="${String(opt[valueKey] || "")}">${String(opt[labelKey] || "")}</option>`).join("");
}
...
function renderPills() {
  ...
  Object.entries(state).forEach(([k, v]) => {
    if (!v || ["compareBy", "compareA", "compareB", "chartType", "groupBy", "metric", "topN"].includes(k)) return;
    labels.push(`<span class="state-pill">${k}: ${v}</span>`);
  });
  container.innerHTML = labels.join("") || `<span class="state-pill">No active filters</span>`;
}

function renderKpis() {
  const kpis = buildKpis(filteredRows);
  el("kpiGrid").innerHTML = kpis.map((kpi) => `<div class="kpi"><div class="kpi-label">${kpi.label}</div><div class="kpi-value">${kpi.value}</div></div>`).join("");
}

function renderInsights() {
  const insights = buildInsights(filteredRows);
  el("insights").innerHTML = insights.map((text) => `<div class="insight-item">${text}</div>`).join("");
}

function renderComparison() {
  const output = el("compareOutput");
  const result = compareSlices(filteredRows, state.compareBy, state.compareA, state.compareB, {
    compareEventId: state.compareEventId,
  });
  const compareEvent = contextData.events.find((e) => e.id === state.compareEventId);
  const compareScope = compareEvent ? compareEvent.name || "Selected event" : "All events";
  output.innerHTML = [
...
function renderCoachView() {
  ...
  if (!athlete) {
    summary.innerHTML = "";
    tableBody.innerHTML = `<tr><td colspan="6">Select an athlete to view coaching analysis.</td></tr>`;
    renderCoachTrendChart([]);
    renderCoachEventChart([]);
    return;
  }
  let rows = filteredRows.filter((r) => r.athleteName === athlete);
  if (eventId) rows = rows.filter((r) => r.eventId === eventId);
  rows.sort((a, b) => (a.date || "").localeCompare(b.date || ""));

  const timeVals = rows.map((r) => r.bestTimeSeconds).filter((v) => Number.isFinite(v));
  const placeVals = rows.map((r) => r.placement).filter((v) => Number.isFinite(v));
  ...
  const avgPlace = placeVals.length ? placeVals.reduce((a, b) => a + b, 0) / placeVals.length : null;
  const recent = rows.slice(-5);
  const trendDelta = recent.length >= 2 && Number.isFinite(recent[0].bestTimeSeconds) && Number.isFinite(recent[recent.length - 1].bestTimeSeconds)
    ? recent[0].bestTimeSeconds - recent[recent.length - 1].bestTimeSeconds
    : null;

  summary.innerHTML = [
    `<div class="kpi"><div class="kpi-label">Athlete</div><div class="kpi-value">${athlete}</div></div>`,
  ...
  if (!rows.length) {
    ...
    tableBody.innerHTML = rows.slice().reverse().slice(0, 50).map((r) => `
      <tr>
        <td>${r.date || "-"}</td>
        <td>${r.eventName}</td>
        <td>${r.meetName}</td>
...
function refreshBuilderSelects() {
  ...
  const scopedRows = state.compareEventId ? filteredRows.filter((r) => r.eventId === state.compareEventId) : filteredRows;
  const values = uniqueValues(scopedRows, field);
  setOptions("compareA", values.map((v) => ({ value: v, label: v })), "value", "label", true, "Select");
  setOptions("compareB", values.map((v) => ({ value: v, label: v })), "value", "label", true, "Select");
...
function populateStaticFilters() {
  setOptions("eventFilter", contextData.events.map((e) => ({ value: e.id, label: e.name || e.id })), "value", "label", true, "All events");
  setOptions("compareEventId", contextData.events.map((e) => ({ value: e.id, label: e.name || e.id })), "value", "label", true, "All events");
  setOptions("meetFilter", contextData.meets.map((m) => ({ value: m.id, label: m.name || m.id })), "value", "label", true, "All meets");
  setOptions("schoolFilter", uniqueValues(allRows, "schoolName").map((v) => ({ value: v, label: v })), "value", "label", true, "All schools");
  setOptions("athleteFilter", uniqueValues(allRows, "athleteName").map((v) => ({ value: v, label: v })), "value", "label", true, "All athletes");
  setOptions("coachAthleteName", uniqueValues(allRows, "athleteName").map((v) => ({ value: v, label: v })), "value", "label", true, "Select athlete");
  setOptions("coachEventId", contextData.events.map((e) => ({ value: e.id, label: e.name || e.id })), "value", "label", true, "All events");
  setOptions("categoryFilter", uniqueValues(allRows, "category").map((v) => ({ value: v, label: v })), "value", "label", true, "All categories");
  refreshBuilderSelects();
...
function bindActions() {
  ...
  el("buildChartBtn").addEventListener("click", () => {
    pullStateFromInputs();
    renderCustomChart(filteredRows, state);
    writeStateToUrl(state);
  });
  el("compareBy").addEventListener("change", () => {
    pullStateFromInputs();
    refreshBuilderSelects();
    renderComparison();
    writeStateToUrl(state);
  });
  el("compareEventId").addEventListener("change", () => {
    pullStateFromInputs();
    refreshBuilderSelects();
    renderComparison();
    writeStateToUrl(state);
  });
  el("setFavoriteBtn").addEventListener("click", () => {
  ...
  el("useFavoriteBtn").addEventListener("click", () => {
  ...
  el("clearFavoriteBtn").addEventListener("click", () => {
    window.localStorage.removeItem(FAVORITE_TEAM_KEY);
    renderFavoriteBanner();
  });
  el("coachApplyBtn").addEventListener("click", () => {
    pullStateFromInputs();
    renderCoachView();
    writeStateToUrl(state);
  });
...
function exportRows(mode) {
  ...
  const cols = ["date", "meetName", "eventName", "athleteName", "schoolName", "gender", "mark", "placement", "heat", "category", "season", "markType", "bestTimeSeconds", "bestFieldMark"];
...
init().catch((error) => {
  const body = el("tableBody");
  if (body) body.innerHTML = `<tr><td colspan="11">Failed to load analytics data: ${error.message}</td></tr>`;
});
