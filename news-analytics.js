...
class MeetAnalytics {
  ...
  async generateMeetSummary(sport, meetId) {
    try {
      ...
      return {
        title: `${meetData.name} — Meet Recap`,
  ...
  _analyzeResults(meetData, results, sport) {
    ...
    topPerformers.forEach(athlete => {
      standoutAthletes.push(
        `${athlete.name || 'Athlete'} (${athlete.team || athlete.school || 'Unknown'}) — ${this._formatPerformance(athlete, sport)}`
      );
    });
  ...
  _formatPerformance(athlete, sport) {
    if (sport === 'track') {
      return `${athlete.event || 'Event'} — ${athlete.time || 'Time TBD'}`;
    } else {
  ...
  _generateHighlights(topPerformers, teamStats, sport) {
    ...
    if (topPerformers.length > 0) {
      highlights.push(
        `🏆 ${topPerformers[0].name || 'Top Athlete'} earned first place`
      );
    }

    if (topPerformers.length > 1) {
      highlights.push(
        `🥈 ${topPerformers[1].name || 'Second Place'} finished strong in second`
      );
    }

    const teamCount = Object.keys(teamStats).length;
    if (teamCount > 0) {
      highlights.push(`🏫 ${teamCount} teams participated`);
    }
